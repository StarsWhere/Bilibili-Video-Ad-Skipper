(function () {
    ...
    const DEFAULT_SETTINGS = {
        ...
        agentPrompt: `你是一个智能agent,专门分析Bilibili视频的弹幕以检测其中包含的商业广告(硬广)时间段。你收到的内容包含两部分: 1. 经过整理后的弹幕文本,格式为 \`MM:SS\` 或 \`HH:MM:SS\` 2. 视频的第一条评论内容及其状态(是否为置顶评论)。你的核心任务是根据这些信息,判断视频是否含有广告,确定广告的时间段,并给出广告概率评估。
...
你的输出**必须且只能是**一个纯粹的、格式正确的JSON对象。**绝对禁止**包含任何JSON之外的文本。`
    ...
    const injectStyles = async () => {
        ...
        const settings = await getStorage('ai_settings', DEFAULT_SETTINGS);
    ...
    const parseAndFilterDanmaku = async (xmlString) => {
    ...
    const showResultPopup = async (result, danmakuSentToAI, commentStringSentToAI) => {
        const settings = await getStorage('ai_settings', DEFAULT_SETTINGS);
        ...
        popup.querySelector('#auto-skip-toggle').addEventListener('change', async (e) => {
            const currentSettings = await getStorage('ai_settings', DEFAULT_SETTINGS);
            await setStorage('ai_settings', { ...currentSettings, defaultSkip: e.target.checked });
    ...
    const main = async () => {
        try {
            ...
            const danmakuText = await parseAndFilterDanmaku(danmakuXml);
            if (!danmakuText) {
                return;
            }
    ...
    // --- FIRST TIME MODAL ---
    const showFirstTimeModal = async () => {
        let currentSettings = await getStorage('ai_settings', DEFAULT_SETTINGS);
        ...
        ftLightBtn.addEventListener('click', async () => {
            currentSettings.theme = 'light';
            await setStorage('ai_settings', currentSettings);
            updateFtThemeVisuals('light');
        });

        ftDarkBtn.addEventListener('click', async () => {
    ...
    const createSettingsUI = async () => {
        ...
        floatingBtn.innerHTML = `<img src="${chrome.runtime.getURL('icons/settings.png')}" alt="AI跳广告">`; // 使用插件内部资源
    ...
    const fetchModels = async (provider, baseUrl, apiKey) => {
        return new Promise((resolve) => {
            ...
            if (fetchableProviders.includes(provider)) {
                ...
                .catch(e => {
                    console.error(`获取 ${provider} 模型列表失败:`, e);
    ...
    const openSettings = async () => {
        let settings = await getStorage('ai_settings', DEFAULT_SETTINGS);
        ...
        document.getElementById('save-btn').addEventListener('click', async () => {
            const newSettings = {
                ...(await getStorage('ai_settings', DEFAULT_SETTINGS)),
            ...
            await setStorage('ai_settings', newSettings);
        ...
        backdrop.addEventListener('click', (e) => {
            if (e.target === backdrop && !isClickInsideModal) {
                backdrop.remove();
            }
            isClickInsideModal = false; // 重置标志
        });
    };
