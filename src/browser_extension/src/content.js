...
**识别广告标记弹幕**:
   ...
   - 注意:忽略含有"正片"、"省流"的弹幕,这些通常指向正常内容, 弹幕不会存在商业推广内容，你只是需要评估是否有类似\`路标\`的弹幕存在即可。有时候，可能会使用中文的\`一二三\`等汉字数字，请注意。
...
4. **时间确定**:
   ...
   - **广告开始时间**: 直接使用所有指向目标时间点的弹幕中最早的弹幕的发送时间，请注意，不是这些弹幕指向的时间，而是这些弹幕出现的时间。如果没有这些信息，可以自行推测。
